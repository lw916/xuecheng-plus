...
public class CoursePublishServiceImpl implements CoursePublishService {
    ...
    public void commitAudit(Long companyId, Long courseId) {
        ...
        if(courseBase == null) XueChengPlusException.cast("找不到该课程");
        //课程审核状态
        ...
        // 查询课程基本信息，营销信息，计划信息等信息插入到课程预发布表
        CourseBaseInfoDto courseBaseInfo = courseBaseInfoService.getCourseBaseById(courseId);
        ...
        if(coursePublishPreUpdate == null){
            ...
            if(insert <= 0){
                XueChengPlusException.cast("无法发布课程请重试");
            }
        }else{
        ...
        if(update < 0) XueChengPlusException.cast("无法修改课程状态请重试");
    }
    ...
    public void uploadCourseHtml(Long courseId, File file) {
        try{
            if(file != null){
                ...
                if(upload == null) {
                    log.error("上传静态文件异常，课程Id:{},无法调用远程接口！", courseId);
                    XueChengPlusException.cast("上传静态文件异常");
                }
            }else{
                log.error("无法获取课程文件的静态文件，课程Id:{}", courseId);
            }
        }catch (Exception e){
            log.error("上传静态文件异常，课程Id:{}", courseId, e);
            e.printStackTrace();
            XueChengPlusException.cast("上传静态文件异常");
        }
    }
